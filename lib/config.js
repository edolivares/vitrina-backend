...
export const resolveCookieConfig = ({ production, secure, sameSite }) => ({
  secure: production || secure === "true",
  sameSite: production ? "none" : sameSite || "lax",
});
...
export const config = {
  ...
  mediaCleanup: {
    ...
    orphanMinAgeHours: parseNumberEnv(process.env.MEDIA_ORPHAN_MIN_AGE_HOURS, 2),
    batchSize: Math.min(parseNumberEnv(process.env.MEDIA_CLEANUP_BATCH_SIZE, 100), 1000),
  },
  metrics: {
