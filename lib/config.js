...
export const config = {
  ...
  storage: {
    ...
    cdnBaseUrl: process.env.CDN_BASE_URL || `http://localhost:${Number(process.env.PORT) || 4000}/storage`,
  },
  s3: {
  ...
  media: {
    avatarMaxFileSizeBytes: parseNumberEnv(process.env.MEDIA_AVATAR_MAX_FILE_SIZE_MB, 2) * 1024 * 1024,
    postMaxFileSizeBytes: parseNumberEnv(process.env.MEDIA_POST_MAX_FILE_SIZE_MB, 10) * 1024 * 1024,
    allowedMimeTypes: parseCsvEnv(process.env.MEDIA_ALLOWED_MIME_TYPES || "image/jpeg,image/png,image/webp"),
  },
  metrics: {
