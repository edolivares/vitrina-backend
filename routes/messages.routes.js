...
router.get("/:chatId/messages", authMiddleware, validateParams(chatIdParamSchema), async (req, res, next) => {
  try {
...
router.post("/:chatId/messages", authMiddleware, validateParams(chatIdParamSchema), validateBody(messageBodySchema), async (req, res, next) => {
  try {
